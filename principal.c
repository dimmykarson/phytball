...
movimento def_movimento(char campo[], int tam, char lado_meu){
    ...
    for(int i = 0;i<qt_movimentos_possiveis;i++){
        ...
        aplica_mov(campo, result, tam, 'e', mov.tipo, mov.posicao, mov.qt_saltos, mov.saltos);
        if(gol(result, tam)) {
              mov_to_do = mov;
              break;
        }
        double v = minimax(result, -1, -INFINITY, INFINITY, MAXD, tam, lado_meu, prox_lado_a_jogar);
        if(v>=aux){
            aux = v;
            mov_to_do = mov;
        }
